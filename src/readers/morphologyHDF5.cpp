...
Property::Properties load(const URI& uri)
{
    try {
        ...
        LBTHROW(morphio::RawDataError("Could not open morphology file " + uri + ": " + exc.what()));
    }
}
...
void MorphologyHDF5::_resolveV1()
{
    ...
    if (_pointsDims.size() != 2 || _pointsDims[1] != _pointColumns) {
        LBTHROW(morphio::RawDataError("Bad number of dimensions in 'points' dataspace"));
    }
    ...
    if (_sectionsDims.size() != 2 || _sectionsDims[1] != _structureV1Columns) {
        LBTHROW(morphio::RawDataError("Bad number of dimensions in 'structure' dataspace"));
    }
}
...
void MorphologyHDF5::_readSectionTypes()
{
    ...
    if (_properties.version() == MORPHOLOGY_VERSION_H5_2) {
        ...
        if (dims.size() != 2 || dims[1] != 1) {
            LBTHROW(MorphioError("': bad number of dimensions in 'sectiontype' dataspace"));
        }
...
void MorphologyHDF5::_readPerimeters(int firstSectionOffset)
{
    ...
    try {
        ...
        if (dims.size() != 1) {
            LBTHROW(MorphioError("': bad number of dimensions in 'perimeters' dataspace"));
        }
